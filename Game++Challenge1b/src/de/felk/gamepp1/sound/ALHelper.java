...
public class ALHelper {
	...
	public static void setPosition(ALSource source, Vector position) throws OpenALException {
		alSource3f(source.getSourceID(), AL_POSITION, position.getX(), position.getY(), -1);
		checkAndThrowALError();
	}
	
	public static void setVelocity(ALSource source, Vector velocity) throws OpenALException {
		alSource3f(source.getSourceID(), AL_VELOCITY, velocity.getX(), velocity.getY(), -1);
		checkAndThrowALError();
	}
	...
	public static void setListener(Vector position, Vector velocity, Vector orientationAt, Vector orientationUp) {
		...
		ori.put(0, orientationAt.getX());
		ori.put(1, orientationAt.getY());
		ori.put(2, -1);
		ori.put(3, orientationUp.getX());
		ori.put(4, orientationUp.getY());
		ori.put(5, -1);
