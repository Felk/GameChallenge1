...
import de.felk.gamepp1.entities.Color;
...
public class RenderHelper {
	...
	public static void renderCircle(float x, float y, float radius, boolean filled) {
		int i;
		...
		if (filled) {
			...
			glVertex3f(x, y, -1); // center of circle
		} else {
			glLineWidth(3);
			glBegin(GL_LINES);
		}
		for (i = 0; i <= triangleAmount; i++) {
			glVertex3f((float) (x + (radius * Math.cos(i * twicePi / triangleAmount))), (float) (y + (radius * Math.sin(i * twicePi / triangleAmount))), -1);
		}
		glEnd();
	}
